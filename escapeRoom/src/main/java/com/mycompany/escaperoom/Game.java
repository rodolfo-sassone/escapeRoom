...
public class Game implements Serializable{
    ...
    public Lose presentation(Scanner in) throws Exception {
        ...
        artGen.printTextArt("           Escape Room", ASCIIArtGenerator.ART_SIZE_SMALL,  ASCIIArtGenerator.ASCIIArtFont.ART_FONT_DIALOG, "▓");
        System.out.print(ansi().fg(CYAN));
        artGen.printTextArt("   Alcatraz", ASCIIArtGenerator.ART_SIZE_LARGE, ASCIIArtGenerator.ASCIIArtFont.ART_FONT_MONO, "■");
        ...
        System.out.println(getCurrentPrisoner().toString());
        System.out.println(getCurrentRoom().getName());
        System.out.println(getCurrentRoom().getDesc());
    ...
    public void saver(PrintStream out)
    {
        ...
        if(score > 0)
        {
            ...
            {
                out.print(ansi().fg(YELLOW));
                out.println("Vuoi salvare il risultato? (S/N)");
                out.print(ansi().fg(CYAN));
                ans = in.nextLine();
            ...
            if(ans.equalsIgnoreCase("s"))
            {
                try {
                    ...
                    {
                        out.print(ansi().fg(YELLOW));
                        out.println("Inserisci un username:");
                        out.print(ansi().fg(CYAN));
                        usr = in.nextLine();
                    }while(usr.isEmpty());
                    
                    Protocol msg = new Protocol(usr, getScore(), date);
                    
                    out.print(ansi().fg(YELLOW));
                    out.println("Connessione al server in corso...");
                    ...
                    try {
                        ObjectOutputStream outStrm = new ObjectOutputStream(s.getOutputStream());
                        ObjectInputStream inStrm = new ObjectInputStream(s.getInputStream());
                        
                        outStrm.writeObject(msg);
                        
                        System.out.println("Salvataggio in corso...");
                        
                        msg = (Protocol) inStrm.readObject();
                        
                        if(msg.getFlag() == true)
                            out.println("Salvataggio eseguito con successo");
                        else
                            out.println("Errore: salvataggio non riuscito");
